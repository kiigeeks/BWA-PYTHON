...
# ==============================================================================
# === FUNGSI BANTUAN & AI (Tidak ada perubahan signifikan) ===
# ==============================================================================
...
def generate_short_report_analysis(tipe_kepribadian, kognitif_utama, pekerjaan, model_ai, bank_data_text, suitability_level: Optional[str] = None, average_score: Optional[float] = None):
    ...
    if not pekerjaan or not pekerjaan.strip():
        ...
        if suitability_level:
            ...
            determined_level = "SESUAI DENGAN CATATAN PENGEMBANGAN"
...
# ==============================================================================
# === FUNGSI-FUNGSI GAMBAR PDF (DIRESTrukturisasi) ===
# ==============================================================================
...
# === HALAMAN BARU SESUAI PERMINTAAN ===
...
def halaman_1_baru(c, biodata, table_data, ai_data, personality_name, cognitive_name, job_name, page_num):
    ...
    # 3. Tabel Analisis Kecocokan (jika ada)
    if table_data:
        y -= 25 # Spasi sebelum tabel
        c.setFont("Times-Bold", 12)
        c.drawString(60, y, f"ANALISIS KECOCOKAN UNTUK: {job_name.upper()}")
        y -= 10 # Spasi setelah judul tabel
        ...
        headers = [
            ...
            Paragraph("Rata-rata (%)", cell_style),
        ...
        col_widths = [5.5*cm, 2.5*cm, 3*cm, 2.5*cm, 3*cm]
        ...
        table.setStyle(TableStyle([
            ...
            ('FONTNAME', (0,0), (-1,0), 'Times-Bold'), ('GRID', (0,0), (-1,-1), 0.5, colors.grey),
            ('BACKGROUND', (0,1), (-1,-1), colors.whitesmoke),
        ]))
        
        w, h = table.wrapOn(c, PAGE_WIDTH - 120, y)
        table.drawOn(c, 60, y - h)
        y -= (h + 20)

    # 4. Bagian Kesesuaian dan Pertimbangan
    ...
    available_space = y - 40 # 40 adalah margin bawah
    if suitability_height > available_space:
        # Jika tidak cukup ruang, gambar di halaman berikutnya
        draw_footer(c, page_num)
        c.showPage()
        page_num += 1
        draw_watermark(c, "cia_watermark.png")
        draw_header(c)
        y = PAGE_HEIGHT - 60 * mm # Reset Y position
    ...
    c.drawCentredString(105*mm, y - 20, f"■ {ai_data.get('suitability', '')}")
    c.drawCentredString(105*mm, y - 35, ai_data.get("position", ''))
    c.setFont("Times-Bold", 12)
    c.drawString(35*mm, y - 55, ai_data.get("reasons_title", ''))
...
def halaman_2_baru(c, topoplot_behavior, topoplot_cognitive, tipe_kepribadian, kognitif_nama, biodata, data_traits, page_num):
    ...
    # 1. Gambar Topoplot
    y = PAGE_HEIGHT - 60*mm
    y = draw_centered_image(c, topoplot_behavior, y, 160)
    ...
    y = draw_centered_image(c, topoplot_cognitive, y - 30, 160)
    ...
    # 2. Kotak Gambar 1 dan 2
    biru_vibrant, kuning_gold = HexColor("#1E88E5"), HexColor("#FCD116")
    biru_border, kuning_border = HexColor("#1565C0"), HexColor("#D4A914")
    box_width, box_gap = 80 * mm, 8 * mm
    start_x = (PAGE_WIDTH - (2 * box_width + box_gap)) / 2
    y_kotak = y - 30 * mm

    p1_temp = Paragraph(data_traits['trait_1_desc'], ParagraphStyle(name='temp1', textColor=white, fontName='Times-Roman', fontSize=10, leading=12, alignment=TA_CENTER))
    w1, h1 = p1_temp.wrapOn(c, box_width - 20, 100*mm)
    box1_height = max(40*mm, h1 + 30)

    p2_temp = Paragraph(data_traits['trait_2_desc'], ParagraphStyle(name='temp2', textColor=white, fontName='Times-Roman', fontSize=10, leading=12, alignment=TA_CENTER))
    w2, h2 = p2_temp.wrapOn(c, box_width - 20, 100*mm)
    box2_height = max(40*mm, h2 + 30)
    
    max_h = max(box1_height, box2_height)

    # Kotak 1
    c.setFillColor(biru_vibrant); c.setStrokeColor(biru_border); c.setLineWidth(1.5)
    c.roundRect(start_x, y_kotak - max_h, box_width, max_h, 5*mm, fill=1, stroke=1)
    c.setFillColor(white); c.setFont("Times-Bold", 12)
    c.drawCentredString(start_x + box_width/2, y_kotak - 18, f"✓ {data_traits['trait_1_title']}")
    p1 = Paragraph(data_traits['trait_1_desc'], p1_temp.style)
    p1.wrapOn(c, box_width - 20, max_h - 30)
    p1.drawOn(c, start_x + 10, y_kotak - 25 - p1.height)
    
    # Kotak 2
    x2 = start_x + box_width + box_gap
    c.setFillColor(biru_vibrant); c.setStrokeColor(biru_border); c.setLineWidth(1.5)
    c.roundRect(x2, y_kotak - max_h, box_width, max_h, 5*mm, fill=1, stroke=1)
    c.setFillColor(white); c.setFont("Times-Bold", 12)
    c.drawCentredString(x2 + box_width/2, y_kotak - 18, f"✓ {data_traits['trait_2_title']}")
    p2 = Paragraph(data_traits['trait_2_desc'], p2_temp.style)
    p2.wrapOn(c, box_width - 20, max_h - 30)
    p2.drawOn(c, x2 + 10, y_kotak - 25 - p2.height)
...
def halaman_3_baru(c, ai_data, page_num):
    ...
    # 1. Area Pengembangan
    merah_muda_border = HexColor("#D4BBA3")
    style_saran = ParagraphStyle(name='saran', fontName='Times-Roman', fontSize=10, leading=14)
    y = PAGE_HEIGHT - 60*mm
    
    total_suggestions_height = sum(Paragraph(f"• {markdown_to_html_platypus(sug)}", style_saran).wrapOn(c, 150*mm, 20*mm)[1] + 4 for sug in ai_data.get('suggestions', []))
    ...
    c.roundRect(25*mm, y - pengembangan_height, 160*mm, pengembangan_height, 5*mm, fill=1, stroke=1)
    c.setFillColor(black); c.setFont("Times-Bold", 13)
    c.drawCentredString(105*mm, y - 20, "Area Pengembangan yang Dapat Dilakukan")
    
    y_saran = y - 35
    for suggestion_md in ai_data.get('suggestions', []):
        p = Paragraph(f"• {markdown_to_html_platypus(suggestion_md)}", style_saran)
        w, h = p.wrapOn(c, 150*mm, 20*mm)
        p.drawOn(c, 30*mm, y_saran-h)
        y_saran -= (h + 4)
    
    # 2. Rekomendasi / Tips Pamungkas
    y = y - pengembangan_height - (7*mm)
    ...
    p_tips_temp = Paragraph(f'{markdown_to_html_platypus(ai_data.get("tips", ""))}', style_tips)
    w_tips, h_tips = p_tips_temp.wrapOn(c, 150*mm, 20*mm)
    tips_height = max(25*mm, h_tips + 10*mm)

    c.setFillColor(biru_vibrant); c.setStrokeColor(biru_border); c.setLineWidth(1.5)
    c.roundRect(25*mm, y - tips_height, 160*mm, tips_height, 5*mm, fill=1, stroke=1)
    ...
    p_tips.drawOn(c, (PAGE_WIDTH-w_tips)/2, y - (tips_height + h_tips)/2)
...
def halaman_5_baru_disclaimer(c, disclaimer_text, page_num):
    ...
    text_width, style = PAGE_WIDTH - 40 * mm, ParagraphStyle(
        ...
        underlineWidth=0.4, underlineOffset= -2.5)
    
    # Perbaikan: Mengganti \n dan spasi ganda sebelum membuat Paragraph
...
# =======================================================
# === FUNGSI UTAMA GENERATE LAPORAN (DIUPDATE) ===
# =======================================================
def generate_short_report(
    ...
    data_traits = {
        "trait_1_title": personality_title, "trait_1_desc": personality_desc,
        "trait_2_title": cognitive_title, "trait_2_desc": cognitive_desc
    }
    ...
    # === PROSES PEMBUATAN PDF DENGAN URUTAN BARU ===
    c = canvas.Canvas(nama_file_output, pagesize=A4)
    next_page_num = 1

    # Halaman 1: Biodata, Tabel, dan Kesimpulan
    next_page_num = halaman_1_baru(c, biodata_kandidat, suitability_table_data, ai_analysis_data,
                                  tipe_kepribadian, kognitif_utama_key, pekerjaan, page_num=next_page_num)

    # Halaman 2: Topoplots dan Deskripsi Trait
    next_page_num = halaman_2_baru(c, topoplot_path_behaviour, topoplot_path_cognitive, tipe_kepribadian,
                                   kognitif_utama_key, biodata_kandidat, data_traits, page_num=next_page_num)
    
    # Halaman 3: Area Pengembangan & Tips
    next_page_num = halaman_3_baru(c, ai_analysis_data, page_num=next_page_num)
    
    # Halaman 4: Person to Fit Job
    next_page_num = halaman_4_baru_job_fit(c, job_fit_data, page_num=next_page_num)
    
    # Halaman 5: Disclaimer
    next_page_num = halaman_5_baru_disclaimer(c, disclaimer_text, page_num=next_page_num)

    c.save()
    print(f"PDF Laporan Pendek '{nama_file_output}' berhasil dibuat dengan layout baru!")